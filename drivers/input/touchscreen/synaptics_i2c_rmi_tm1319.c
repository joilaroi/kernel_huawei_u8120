...
int buf_len =13;
	...
	for (i = 0; i < ((ts->use_irq && !bad_data) ? 1 : 10); i++) {
		...
		if (ret < 0) {
			...
			SYNAPITICS_DEBUG("buf[buf_len]=%d,buf_len=%d", buf[buf_len], buf_len);
			if ((buf[8] & 0xc0) != 0x40) {
				printk(KERN_WARNING "synaptics_ts_work_func:"
				       " bad read %x %x %x %x %x %x %x %x %x"
				       " %x %x %x %x %x %x %x %x %x %x %x, ret %d\n",
				       buf[0], buf[1], buf[2], buf[3],
				       buf[4], buf[5], buf[6], buf[7],
				       buf[8], buf[9], buf[10], buf[11],
				       buf[12], buf[13], buf[14], buf[15], buf[16], buf[17], buf[18], buf[19], ret);
				if (bad_data)
					printk(KERN_WARNING "bad_data=1");
			...
			{
				//printk(KERN_DEBUG "synaptics_ts_work_func: good read ");
				//int j;
				//for(j=0;j<512;j++)
				//	printk("%x ",buf[j]);
				//printk("\n");
/*
				       " good read %x %x %x %x %x %x %x %x %x"
...
, ret);*/
				...
				int f, a;
				int base;
				//int x = buf[5] | (uint16_t)(buf[4] & 0x1f) << 8; 
				//int y = buf[3] | (uint16_t)(buf[2] & 0x1f) << 8;
				int z = buf[1];
				int w = buf[0] >> 4;
				int finger = buf[0] & 7;
				...
				for (f = 0; f < 2; f++) {
					...
					for (a = 0; a < 2; a++) {
						...
						//SYNAPITICS_DEBUG("f=%d,a=%d,p=%d,flip_flag=%d,ts->flags=%d,ts->max[a]=%d",f,a,p,flip_flag,ts->flags,ts->max[a]);
				...
				if (z) {
					input_report_abs(ts->input_dev, ABS_X, pos[0][1]);
					input_report_abs(ts->input_dev, ABS_Y, 5880-pos[0][0]);
				}
				...
				if (finger2_pressed) {
					input_report_abs(ts->input_dev, ABS_HAT0X, pos[1][1]);
					input_report_abs(ts->input_dev, ABS_HAT0Y, 5880-pos[1][0]);
				}
				...
				input_report_abs(ts->input_dev, ABS_MT_POSITION_X, pos[0][1]);
				input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, 5880-pos[0][0]);
				input_mt_sync(ts->input_dev);
				if (finger2_pressed) {
					input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, z);
					input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, w);
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X, pos[1][1]);
					input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, 5880-pos[1][0]);
					input_mt_sync(ts->input_dev);
